...
SmtkKeysMapper *smtk_keys_mapper_new(bool show_shift, bool hide_visible, const char *layout,
				     const char *variant, GError **error)
{
	SmtkKeysMapper *mapper =
		g_object_new(SMTK_TYPE_KEYS_MAPPER, "show-shift", show_shift, "hide-visible", hide_visible,
...
static bool is_modifier_active(struct xkb_state *xkb_state)
{
	return !xkb_state_mod_names_are_active(xkb_state, XKB_STATE_MODS_EFFECTIVE, XKB_STATE_MATCH_ANY, XKB_MOD_NAME_LOGO, XKB_MOD_NAME_CTRL, XKB_MOD_NAME_ALT, XKB_MOD_NAME_SHIFT, NULL);
}

static char *smtk_keys_mapper_concat_key(SmtkKeysMapper *mapper, SmtkKeyMode mode, char *main_key)
{
	...
	if (xkb_state_mod_name_is_active(mapper->xkb_state, XKB_MOD_NAME_LOGO,
		...
		g_string_append(buffer, mode == SMTK_KEY_MODE_COMPACT ? "⌘" : "Super+");
	if (xkb_state_mod_name_is_active(mapper->xkb_state, XKB_MOD_NAME_CTRL,
					 XKB_STATE_MODS_EFFECTIVE) > 0)
		g_string_append(buffer, mode == SMTK_KEY_MODE_COMPACT ? "⌃" : "Ctrl+");
	...
	if (xkb_state_mod_name_is_active(mapper->xkb_state, XKB_MOD_NAME_ALT,
		...
		g_string_append(buffer, mode == SMTK_KEY_MODE_COMPACT ? "⌥" : "Alt+");
		...
		if (mapper->show_shift ||
		    !xkb_state_mod_index_is_consumed(
			    ...
			    xkb_keymap_mod_get_index(mapper->xkb_keymap,
						     XKB_MOD_NAME_SHIFT)))
			g_string_append(buffer, mode == SMTK_KEY_MODE_COMPACT ? "⇧" : "Shift+");
	g_string_append(buffer, main_key);
	g_free(main_key);
	return g_string_free(buffer, FALSE);
}
static char *smtk_keys_mapper_get_key(SmtkKeysMapper *mapper, SmtkKeyMode mode, SmtkEvent *event)
{
	char *main_key = NULL;
	...
	if (mapper->hide_visible && !is_modifier_active(mapper->xkb_state)) {
		g_free(main_key);
		return NULL;
	}
	const char *replace_name =
		g_hash_table_lookup(mode == SMTK_KEY_MODE_COMPACT ? mapper->compact_replace_names : mapper->composed_replace_names, main_key);
...
void smtk_keys_mapper_set_hide_visible(SmtkKeysMapper *mapper, bool hide_visible)
{
	g_return_if_fail(mapper != NULL);
