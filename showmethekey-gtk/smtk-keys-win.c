...
static void smtk_keys_win_constructed(GObject *object)
{
	...
	win->emitter = smtk_keys_emitter_new(
		win->show_shift, win->show_keyboard, win->show_mouse, win->hide_visible, win->mode,
...
GtkWidget *smtk_keys_win_new(SmtkAppWin *app_win, bool show_shift,
			     ...
			     bool draw_border, bool hide_visible, SmtkKeyMode mode, int width,
			     ...
			     const char *variant, GError **error)
{
	SmtkKeysWin *win = g_object_new(
		...
		"draw-border", draw_border, "hide-visible", hide_visible, "timeout", timeout, "layout",
